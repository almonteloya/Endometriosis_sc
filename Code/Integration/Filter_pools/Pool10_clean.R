### Cleaning pool 10
## We are going to filter everything that overlaps with cluster 0 assigned by freemuxlet,
## which according to the heatmap looks assigned to a different patient. 

library(Seurat)
library(dplyr)
project="XREP1a"
x=10
print(paste("LOADING SAMPLE",x))
sample=paste0('XREP1-POOL-SCG',x)
parent_dir= "insert/path/here/"
in_dir = sprintf("%s/%s/automated_processing_filter/", parent_dir,sample)
...
sobj = subset(x = sobj, idents = c(1,2,3))
print(Seurat::DimPlot(sobj, reduction = "umap",label = TRUE))
sobj <- RunUMAP(sobj,
...
####---- Filtering--- looking at the heatmap we see that cluster 6 is junk

#number of clusters
selected_c<-levels(sobj$seurat_clusters)[levels(sobj$seurat_clusters)!=6]
sobj<-subset(sobj, idents = selected_c)
sobj <- RunUMAP(sobj, dims = 1:25)
saveRDS(sobj, file = paste0(in_dir,sample,"_scTransformed_processed_filtered.RDS"))
