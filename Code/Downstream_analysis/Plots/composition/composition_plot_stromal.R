...
s_obj = readRDS(file=paste0(dir_in,"Stromal_obj_2025.RDS"))
...
make_meta_barplot(subset_cell, condition_full, annotation_updated, dittoColors())+
...
make_meta_barplot(subset_cell, condition_control, annotation_updated, dittoColors())+
...
make_meta_barplot(subset_cell, Phase_menstrual_reduced, annotation_updated, dittoColors())+
...
make_meta_barplot(subset_cell, LIBRARY, annotation_updated, dittoColors())+
...
make_meta_barplot(subset_cell, Patient_id, annotation_updated, dittoColors())+
  scale_fill_manual(values=dittoColors())+
  ylab("fraction of cluster") + theme(legend.text = element_text(size=15),
                                      axis.text=element_text(size=15),)
dev.off()
